  ...
  const rows = (row, rowIndex) => {
    for (let [columnIndex, squareContent] of row.entries()) {
      document.getElementById(
        `square${rowIndex}${columnIndex}`).innerText = squareContent
    }
  }
  return {
    refresh () {
...
const playRound = (() => {
  const startListening = () => {
    document.querySelector('.game-board')
      .addEventListener('click', listenToGameBoard)
    document.querySelector('.game-board').style.cursor = 'pointer'
  }

  const stopListening = () => {
    document.querySelector('.game-board')
      .removeEventListener('click', listenToGameBoard)
...
const markSquare = (id) => {
  ...
  if (gameBoard.grid[rowIndex][columnIndex] === '') {
    ...
    document.getElementById(
      `square${rowIndex}${columnIndex}`).style.cssText = 'cursor: default'
  }
...
const determineWinner = () => {
    ...
    for (const item of checkPattern) {
      ...
      const highlightWinningSquares = (color) => {
        item.forEach(element => {
          document.getElementById(
            `square${element[0]}${element[1]}`).style.background = color
        })
      }
  ...
  if (players.player1.playerName === players.ai1.playerName &&
    gameBoard.currentTurn === 'X') {
    aisTurn(1)
  } else if (players.player2.playerName === players.ai2.playerName &&
    gameBoard.currentTurn === 'O') {
    aisTurn(2)
  }
}
...
const startGame = (name1, name2) => {
  ...
  if (aiCheckBox[0].checked) {
    try {
      sessionStorage.getItem('savedPlayer1') ? document.getElementById(
  ...
  if (aiCheckBox[1].checked) {
    try {
      sessionStorage.getItem('savedPlayer2') ? document.getElementById(
        'player-2-input').value
  ...
  if (typeof (Storage) !== 'undefined') {
    if (players.player1.playerName !==
      players.ai1.playerName) sessionStorage.setItem('savedPlayer1',
      players.player1.playerName)
    if (players.player2.playerName !==
      players.ai2.playerName) sessionStorage.setItem('savedPlayer2',
      players.player2.playerName)
  }
  if (aiDisplayControls.difficultyLevel1 === undefined ||
    aiDisplayControls.difficultyLevel1 === null
    || aiDisplayControls.difficultyLevel1 === '') {
    document.getElementById('medium-button-1').click()
  }
  if (aiDisplayControls.difficultyLevel2 === undefined ||
    aiDisplayControls.difficultyLevel2 === null
...
const setDifficultyLevel = (() => {
  const makeEasy = (playerNumber) => {
    ...
    const mediumButton = document.getElementById(
      `medium-button-${playerNumber}`)
    const impossibleButton = document.getElementById(
      `impossible-button-${playerNumber}`)
...
const easyAIStrategy = (maxDepth) => {
  ...
  if (gameBoard.grid[xCoordinate][yCoordinate] != 'X' &&
    gameBoard.grid[xCoordinate][yCoordinate] != 'O') {
    setTimeout(function () {
...
const impossibleAIStrategy = () => {
  ...
  const flatten2DGrid = (grid) => {
    ...
    for (let i = 0; i < grid.length * grid[0].length; i++) {
      const currentValue = grid[i % grid[0].length][Math.floor(
        i / grid[0].length)]
...
// ---------- Start by clicking on game board
const startByBoardClick = () => {
  document.querySelector('.game-board')
